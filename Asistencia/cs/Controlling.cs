...
namespace CaniaBrava
{
    class Controlling
    {
        ...
        public void procesa(System.ComponentModel.BackgroundWorker worker, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            try
            {
                query = @"INSERT INTO controlling_version VALUES ('" + Tipo + "','" + Anio + "','" + Anio + "-" + (Mes == string.Empty ? "01" : Mes) + "-01" + "','" + Versi + @"');

INSERT INTO controlling SELECT *," + (Versi - 1) + " FROM controlling_final WHERE Tipo='" + Tipo + Anio.Substring(2, 2) + @"';

DELETE FROM controlling_final WHERE Tipo='" + Tipo + Anio.Substring(2, 2) + "';";
            ...
            using (OleDbConnection con = new OleDbConnection(ConnString))
            {
                try
                {
                    ...
                    using (SqlBulkCopy bulkCopy = new SqlBulkCopy(sqlConnectionString))
                    {
                        ...
                        bulkCopy.DestinationTableName = "controlling_final";
                ...
                catch (Exception ex)
                {
                    ...
                    try
                    {
                        query = "DELETE FROM controlling_version WHERE anio='" + Anio + "' AND tipo='" + Tipo + "' AND versi='" + Versi + @"';

INSERT INTO controlling_final 
...
FROM controlling WHERE Tipo='" + Tipo + Anio.Substring(2, 2) + "' AND version='" + (Versi - 1) + @"';

DELETE FROM controlling WHERE Tipo='" + Tipo + Anio.Substring(2, 2) + "' AND version='" + (Versi - 1) + "'";
