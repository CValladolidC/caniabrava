...
namespace CaniaBrava
{
    class BoletaHxtra
    {
        ...
        public string generaBoletaTabular(string idperplan, string anio, string messem, string idcia, string idtipoper, string idtipoplan, string idtipocal)
        {
           ...
           // query = query + " as tipocuentarem,A.nroctarem, ifnull(dtp.pervac,' ') AS anio_vaca from plan_ P inner join conbol X on "; 
           ...
           // query = query + " and X.idconplan=Z.idconplan inner join conplan M on M.idcia=Z.idcia and ";
           // query = query + " M.idtipoplan=Z.idtipoplan and M.idtipocal=Z.idtipocal and M.idconplan=Z.idconplan";
           ...
           // query = query + " and P.idtipocal=S.idtipocal and P.idcia=S.idcia left join fonpen T on P.idfonpen=T.idfonpen";
           ...
           // query = query + " left join maesgen F on F.idmaesgen='007' and A.entfinrem=F.clavemaesgen left join maesgen G on G.idmaesgen='014' and A.tipctarem=G.clavemaesgen";
           ...
           // query = query + " and P.idtipoplan='" + @idtipoplan + "' and P.idtipocal='" + @idtipocal + "' and Z.imprime='SI' and X.valor>0 order by P.idperplan asc,Y.orden asc,X.idconplan asc;";
            String query = " call generaBoletaTabular('"+ @idperplan+"','" +@anio+ "','" +@messem+"','"+ @idcia +"','"+ @idtipoper+"','"+@idtipoplan+"','"+ @idtipocal+"', 'HXTRA')";
            ...
            if (boleta.Rows.Count > 0)
            {
                ...
                foreach (DataRow row_boleta in boleta.Rows)
                {
                    ...
                    if (nrodatarow.Equals(0))
                    {
                        ...
                        arrNomb = nombre.Trim().Split(',');

                        a = "P A G O   D E   H O R A S   E X T R A S     " + "\n";
                        b = (row_boleta["ruccia"].ToString() + fn.replicateCadena(" ", 1) + descia + fn.replicateCadena(" ", 40)).Substring(0, 40) + fn.replicateCadena(" ", 2) + "PERIODO: " + row_boleta["messem"].ToString() + "/" + row_boleta["anio"].ToString() + "\n";
                        c = "CODIGO: " + (row_boleta["idperplan"].ToString() + fn.replicateCadena(" ", 65)).Substring(0, 56) + fn.replicateCadena(" ", 2) + "OCUPACION : " + ocupacion.Substring(0, 40) + fn.replicateCadena(" ", 2) + "\n";
                        d = "NOMBRE: " + (arrNomb[0] + arrNomb[1].Split(' ')[1] + fn.replicateCadena(" ", 40)).Substring(0 , 37) + fn.replicateCadena(" ", 2) + "DOC.IDENT. :" + row_boleta["cortotipodoc"].ToString() + fn.replicateCadena(" ", 2) + row_boleta["nrodoc"].ToString() + "\n";
                        e = fn.replicateCadena("-", 120) + "\n";
                        f = "                                    H O R A S               E X T R A S                " + "\n";
                        g = fn.replicateCadena("-", 120) + "\n";
                        strPrinter = a + b + c + d + e + f + g;
                    ...
                    else
                    {
                        if (idclascol == "I")
                        {
                            ...
                            sumHxtras += float.Parse(row_boleta["valor"].ToString());
                ...
                if (dtdetalle.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow row_dtdetalle in dtdetalle.Rows)
                    {
                        ...
                        h = desboletapar.Substring(0, 18) + fn.replicateCadena(" ", 1) + valorpar.Substring(0, 8);
                        i = fn.replicateCadena(" ", 2) + desboletaing.Substring(0, 18) + fn.replicateCadena(" ", 1) + valoring.Substring(0, 8);
                        //fn.replicateCadena(" ", 2) + desboletades.Substring(0, 18) + fn.replicateCadena(" ", 1) + valordes.Substring(0, 8);
                        j = fn.replicateCadena(" ", 2) + desboletaapor.Substring(0, 18) + fn.replicateCadena(" ", 1) + valorapor.Substring(0, 8) + "\n";
                        strPrinter += h + i + j;
                        nrofiladetalle++;
                    }
                }

                k = "\n" + fn.replicateCadena("-", 120) + "\n";
                l = fn.replicateCadena(" ", 90) + "Total Horas Extras     : " + sumHxtras.ToString("##,##0.00;(##,##0.00);Zero") + "\n";
                m = fn.replicateCadena("-", 120) + "\n"; //+ "\n";
                n = fn.replicateCadena(" ", 12) + descia_va + fn.replicateCadena(" ", 10) + "\n";
                o = fn.replicateCadena(" ", 7) + fn.replicateCadena("-", 50) + fn.replicateCadena(" ", 15) + fn.replicateCadena("-", 25) + "\n";
                p = fn.replicateCadena(" ", 22) + "EMPLEADOR" + fn.replicateCadena(" ", 40) + "TRABAJADOR";
                strPrinter += k + l + m + n + o + p;
            }
