...
{
    public partial class ui_addcapacitados : Form
    {
        ...
        public void EditarProg(string idprog, string idestado)
        {
            ...
            var listaEmpleados = ListaPerPlan();
            var lista = ListaProgDet(idprog);

            this.Fecha01 = lista.Min(x => x.fechaini);
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            if (Validar())
            {
                ...
                foreach (var item in listaTipHorario)
                {
                    var det = listdet.Where(c => c.fechadiaria.DayOfWeek.ToString().ToUpper() == item.dias_en).ToList();
        ...
        private int VerificarDatosDupli(List<programacion> listdet)
        {
            ...
            //query += "WHERE a.idperplan in ('01537258','00366900') and a.fechadiaria in ('2020-07-30');";
        ...
        {
            if (string.IsNullOrEmpty(txtProg.Text))
            {
                MessageBox.Show("Ingresa un nombre de Programacion", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                txtProg.Focus(); return false;
            }
            return true;
        }
        #endregion
