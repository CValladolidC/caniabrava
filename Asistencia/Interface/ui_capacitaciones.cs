...
{
    public partial class ui_capacitaciones : Form
    {
        GlobalVariables variables = new GlobalVariables();
        Funciones funciones = new Funciones();

        private ui_updcapacitaciones form = null;

        private ui_updcapacitaciones FormInstance
        {
            get
            {
                if (form == null)
                {
                    form = new ui_updcapacitaciones();
        ...
        private void ui_ListaProg()
        {
            ...
            {
                if (tema) { query += "WHERE a.descapacita LIKE '%" + buscar + "%'"; }
        ...
        {
            ui_updcapacitaciones ui_detalle = this.FormInstance;
        ...
        private void btnAprobar_Click(object sender, EventArgs e)
        {
            ...
            if (selectedCellCount > 0)
            {
                ...
                string fecini = dgvdetalle.Rows[dgvdetalle.SelectedCells[1].RowIndex].Cells[2].Value.ToString();
                string fecfin = dgvdetalle.Rows[dgvdetalle.SelectedCells[1].RowIndex].Cells[3].Value.ToString();
                string apro1 = dgvdetalle.Rows[dgvdetalle.SelectedCells[1].RowIndex].Cells[8].Value.ToString();
                string apro2 = dgvdetalle.Rows[dgvdetalle.SelectedCells[1].RowIndex].Cells[9].Value.ToString();
                string apro3 = dgvdetalle.Rows[dgvdetalle.SelectedCells[1].RowIndex].Cells[10].Value.ToString();
                string estado = dgvdetalle.Rows[dgvdetalle.SelectedCells[1].RowIndex].Cells[16].Value.ToString();
                ...
                switch (variables.getValorNivelUsr())
                {
                    case "2": usu = apro1; break;
                    case "3": usu = apro2; break;
                    case "4": usu = apro3; break;
                }

                if (estado == "PENDIENTE" && usu == string.Empty)
                {
                    ...
                    if (confirmResult == DialogResult.Yes)
                    {
                        ...
                        {
                            case "2":
                                query += "aprobador1='" + variables.getValorUsr() + "', estaprobador1='3',fecaprobador1=GETDATE() ";
                                break;
                            case "3":
                                query += "aprobador2='" + variables.getValorUsr() + "', estaprobador2='3',fecaprobador2=GETDATE() ";
                                break;
                            case "4":
                                query += "aprobador3='" + variables.getValorUsr() + "', estaprobador3='3',fecaprobador3=GETDATE(),estado='3' ";
                                break;
                        }
