...
{
    public partial class ui_programacionGraf : Form
    {
        //Oliver Cruz Tuanama
        ...
        private ui_updprogramacionGraf form = null;

        private ui_updprogramacionGraf FormInstance
        {
            get
            {
                if (form == null)
                {
                    form = new ui_updprogramacionGraf();
        ...
        private void ui_ListaProg()
        {
            ...
            {
                query += " and a.idusrins IN (SELECT DISTINCT idusr FROM cencosusr (NOLOCK) WHERE idcencos in (";
                query += "SELECT idcencos FROM cencosusr (NOLOCK) WHERE idusr = '" + variables.getValorUsr() + "')) ";
            }
        ...
        {
            ui_updprogramacionGraf ui_detalle = this.FormInstance;
        ...
        private void btnReprogramar_Click(object sender, EventArgs e)
        {
            ...
            if (selectedCellCount > Convert.ToInt32(0))
            {
                ...
                if (Fec2.Date >= DateTime.Now.Date)
                {
                    ...
                    if (confirmResult == DialogResult.Yes)
                    {
                        ui_reprogramacionInicial ui_detalle = new ui_reprogramacionInicial();
